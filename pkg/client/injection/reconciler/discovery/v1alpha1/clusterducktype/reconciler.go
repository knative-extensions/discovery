...
// controller reconciling v1alpha1.ClusterDuckType.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ClusterDuckType. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ClusterDuckType) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ClusterDuckType.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ClusterDuckType. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ClusterDuckType) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ClusterDuckType if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ClusterDuckType.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ClusterDuckType) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ClusterDuckType if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.ClusterDuckType.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.ClusterDuckType) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.ClusterDuckType) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.ClusterDuckType resources.
type reconcilerImpl struct {
	...
	Lister discoveryv1alpha1.ClusterDuckTypeLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister discoveryv1alpha1.ClusterDuckTypeLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.ClusterDuckType, desired *v1alpha1.ClusterDuckType) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.DiscoveryV1alpha1().ClusterDuckTypes()
		...
		updater := r.Client.DiscoveryV1alpha1().ClusterDuckTypes()
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ClusterDuckType) (*v1alpha1.ClusterDuckType, error) {
	...
	patcher := r.Client.DiscoveryV1alpha1().ClusterDuckTypes()
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ClusterDuckType) (*v1alpha1.ClusterDuckType, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ClusterDuckType, reconcileEvent reconciler.Event) (*v1alpha1.ClusterDuckType, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
